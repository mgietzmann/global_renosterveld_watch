...
class _KeyedTFRecordSource(FileBasedSource):
    ...
    def __init__(self, file_pattern, coder, compression_type, validate):
        ...
        super(_KeyedTFRecordSource, self).__init__(
...
class KeyedReadFromTFRecord(PTransform):
    ...
    def __init__(
        ...
        self._source = _KeyedTFRecordSource(
...
with beam.Pipeline() as p:
    ...
    def break_up_patch(x):
        key, data_dict = x
        return [(f'{key},{i}', pixel_dict) for i, pixel_dict in enumerate(tf.data.Dataset.from_tensor_slices(data_dict))]
    ...
    data = (
        p
        | 'Read Earth Engine Output' >> KeyedReadFromTFRecord('gs://reno-ee-example/Image_reno_predict__2021-07-04.tfrecord.gz')
        ...
        | 'Transform Data Dict Into a Timeline of Tuples' >> beam.Map(pass_key(lambda data_dict: tf.transpose(list(data_dict.values()))))
        | 'JSONify Data' >> beam.Map(jsonify_data)
        | 'Write Data' >> beam.io.WriteToText('gs://reno-ee-example/TestBeamOutput.txt')
    )