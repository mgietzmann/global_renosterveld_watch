...
def main(event, context):
    ...
    if match is not None:
        ...
        input_pattern = f'gs://grw-predict/{match.groups()[0]}*'
        ...
        request = service.projects().locations().flexTemplates().launch(
            ...
            body={
                'launch_parameter': {
                    'jobName': f"grw-recombine-{int(time())}",
                    'parameters': {
                        "input": input_pattern,
                        "output": f"gs://grw-recombine/{int(time())}/",
                        "max_num_workers": "50",
                        "worker_machine_type": "n1-standard-2"
                    },
                    'containerSpecGcsPath': "gs://grw-recombine/dataflow/templates/grw_recombine.json"
                }
            }
        )
